...
export function validateProspect(data: Record<string, unknown>): { valid: boolean; errors: string[] } {
  ...
  if (data.targetSalary !== undefined && data.targetSalary !== null) {
    if (typeof data.targetSalary !== "string" || data.targetSalary.trim() === "") {
      errors.push("Target salary must be a non-empty string when provided");
    }
  }

  if (data.followUpDate !== undefined && data.followUpDate !== null) {
    const ISO_DATE_REGEX = /^\d{4}-\d{2}-\d{2}$/;
    if (typeof data.followUpDate !== "string" || !ISO_DATE_REGEX.test(data.followUpDate)) {
